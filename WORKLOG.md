...
- **管理タブの表示制限**: ユーザー指示により、管理画面（全件一覧・科目マスタ・承認ルート設定等）を「本部・社長のみ」に限定（従来は`is_master`フラグでも表示されていたのを除外し、`role==='CEO'||role==='HQ'`のみに変更）。※DB側のRLS（`expense_can_manage()`）は変更していないため、is_masterアカウントは引き続きAPI経由では操作可能（画面上に導線が出ないだけ）。一時ユーザー（is_master=true・role=TEAM）で管理タブが非表示、role=HQに変更後は表示されることを実機確認
